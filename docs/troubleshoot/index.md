...
You might see `[ERROR][c.a.o.s.s.t.OpenDistroSecuritySSLNettyTransport] [odfe-node1] SSL Problem Insufficient buffer remaining for AEAD cipher fragment (2). Needs to be more than tag size (16)` when starting Open Distro. This problem is a [known issue with Java](https://bugs.openjdk.java.net/browse/JDK-8221218) and doesn't affect the operation of the cluster.
...
If you run `sudo yum upgrade` and receive a dependency error, Elasticsearch OSS likely has a new minor version that the Open Distro plugins don't support yet. You can [install a specific, supported version of Elasticsearch OSS](../install/plugins/#compatibility) to resolve the issue.
...
If Elasticsearch OSS fails to start and you're using Java 8, verify that you set the symbolic link (symlink) correctly in [step 6](../install/rpm) of the RPM installation. If Java is installed to a non-standard path, try looking for `tools.jar` using the following command:
