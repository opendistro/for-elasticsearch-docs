...
You can convert your full-text queries into a search template to take in user input and dynamically insert it into your query.
...
{
  "source": {
    "from": "{% raw %}{{page}}{{^page}}10{{/page}}{% endraw %}",
...
Once you have the search template working like the way you want, you can save the source of that template as a script making it reusable for different input parameters.

When saving the search template as a script, you need to specify the `lang` parameter to be `mustache`.

This command saves the search template as a script:
...
You can specify conditions, run loops, join arrays, convert to JSON, and so on.
...
```
{% raw %}{{#var}}conditional text{{/var}}{% endraw %}
```

When `var` is a boolean value, this syntax acts like an `if` condition. The `{% raw %}{{#var}}{% endraw %}` and `{% raw %}{{/var}}{% endraw %}` tags insert the values placed between them only if `var` evaluates to `true`.
...
Using section tags would make your JSON invalid, so you must instead write your query in a string format:
...
This command sets `size` to `2` if `limit` is `true` and sets it to `10` otherwise:
...
You can use the section tag to also implement a `foreach` loop:

```
{% raw %}{{#var}}{{.}}}{{/var}}{% endraw %}
```
When `var` is an array, the search template iterates through them represented by {% raw %}{{.}}{% endraw %} and creates a terms query.
...
You can use tag to concatenate the values of an array separated by commas:
...
GET _search/template
{
  "source": {
    "query": {
      "match": {
        "play_name": "{{#join}}play_name{{/join}}"
      }
    }
  },
  "params": {
    "emails": [ "Henry IV", "Othello" ]
  }
}
```
...
You can use tag to to convert parameters to their JSON representation:

```json
GET _search/template
{
  "source": "{\"query\":{\"terms\":{\"play_name\":[\"{{#toJson}}\",\"play_name\",\"{{\/toJson}}\"]}}}",
  "params": {
    "play_name": [ "Henry IV", "Othello" ]
  }
}
