...
Sibling aggregations take the output of a sibling aggregation and produce new buckets or new aggregations at the same level as the sibling buckets.
...
`min_bucket`, `max_bucket`, `sum_bucket`, and `avg_bucket` aggregations are common sibling aggregations.

### Parent aggregations

Parent aggregations take the output of a parent aggregation and produce new buckets or new aggregations at the same level as the existing buckets.
...
`derivatives` and `cumulative_sum` aggregation are common parent aggregations.
...
The following example creates a date histogram with a one-month interval. The `sum` sub-aggregation calculates the sum of all bytes for each month. Finally, the `avg bucket` aggregation uses this sum to calculate the average number of bytes per month.
...
The following example uses the sum aggregation on the buckets generated by the date histogram. From the resultant buckets values, the percentage of RAM is calculated in an interval of 10,000 bytes in the context of a zip extension:
...
For example, you can use the cumulative sum aggregation to calculate the cumulative number of bytes over a monthly basis:
...
The first `cumulative_bytes` field is the sum of the first two buckets. The second `cumulative_bytes` field adds this result to the value of the next bucket. In this way, it accumulates the sums of all buckets in sequence.
...
In mathematics, the derivative of a function measures its sensitivity to change. In other words, a derivative evaluates the rate of change in some function with respect to some variable. To learn more about derivates, see https://en.wikipedia.org/wiki/Derivative.
...
The 2nd order derivative is a double derivative or the derivative of the derivative.
The 2nd order derivative indicates how the rate of change of a quantity is itself changing. It’s the difference between the 1st order derivatives of adjacent buckets.
...
The first bucket doesn't have a 1st order derivate as a derivate needs at least two points for comparison. The first and second buckets don't have a 2nd order derivate because a 2nd order derivate needs at least 2 data points from the 1st order derivative.
...
For more information, see https://en.wikipedia.org/wiki/Moving_average.
...
The `moving_avg` aggregation supports five models - `simple`, `linear`, `exponentially weighted`, `holt-linear`, and `holt-winters`. These models differ in how the values of the window are weighted. As data points become "older" (i.e., the window slides away from them), they might be weighted differently. You can specify a model of your choice by setting the model property; this property holds the name of the model and the settings object, which you can use to provide model properties.

For more information on these models, see https://en.wikipedia.org/wiki/Moving_average.
...
You can use the `serial_diff` aggregation to draw the data changes between time periods instead of drawing the whole value.
...
For example, You know that the population of a city grows with time. If we use the serial differencing aggregation with the period of one day, we can see the daily growth. For example, you can compute a series of the differences of the weekly average changes of a total price.
