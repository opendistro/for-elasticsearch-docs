...
```bash
docker pull amazon/opendistro-for-elasticsearch:1.10.0
docker pull amazon/opendistro-for-elasticsearch-kibana:1.10.0
```
...
```bash
docker run -p 9200:9200 -p 9600:9600 -e "discovery.type=single-node" amazon/opendistro-for-elasticsearch:1.10.0
```
...
amazon/opendistro-for-elasticsearch:1.10.0
```
