...
**5.** The **DSN** comes already setup with the installation. The following connection information is setup as part of default DSN:
...
The DSN can be customized by using **ODBC Data Source Administrator** which is pre-installed on Windows10.
...
**7.** Currently the **DSN** is not setup as part of installation and need to be configured manually.
...
The driver comes in the form of a library file: `odfesqlodbc.dll` for Windows and `libodfesqlodbc.dylib` for MacOS.
