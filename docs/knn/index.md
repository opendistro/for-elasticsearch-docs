...
In Elasticsearch, codecs handle the storage and retrieval of indices. Under the hood, the KNN plugin uses a custom codec to writes vector data to a graph so that the underlying KNN search library can read it.
{: .tip }
